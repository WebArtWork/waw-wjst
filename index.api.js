...
module.exports = function (waw) {
  ...
  waw.build = function (root, page) {
    ...
    if (!fs.existsSync(indexPath)) {
      return console.log("Missing index.html in wjst " + root + " root folder");
    }
    ...
    if (
      ...
      code = code.replace(
        "<!-- CSS -->",
        fs.readFileSync(path.join(root, "pages", page, "index.css"), "utf8")
      );
    } else if (fs.existsSync(path.join(root, "pages", page, page + ".css"))) {
      code = code.replace(
        "<!-- CSS -->",
        fs.readFileSync(path.join(root, "pages", page, page + ".css"), "utf8")
      );
    ...
    if (fs.existsSync(path.join(root, "pages", page, "index.html"))) {
      code = code.replace(
        "<!-- HTML -->",
        fs.readFileSync(path.join(root, "pages", page, "index.html"), "utf8")
      );
    } else if (fs.existsSync(path.join(root, "pages", page, page + ".html"))) {
      code = code.replace(
        "<!-- HTML -->",
        fs.readFileSync(path.join(root, "pages", page, page + ".html"), "utf8")
      );
    ...
    if (fs.existsSync(path.join(root, "pages", page, "index.js"))) {
      code = code.replace(
        "<!-- JS -->",
        fs.readFileSync(path.join(root, "pages", page, "index.js"), "utf8")
      );
    } else if (fs.existsSync(path.join(root, "pages", page, page + ".js"))) {
      code = code.replace(
        "<!-- JS -->",
        fs.readFileSync(path.join(root, "pages", page, page + ".js"), "utf8")
      );
  ...
  const resolveImports = (html, basePath) => {
    return html.replace(/<!--\s*IMPORT:\s*(.*?)\s*-->/g, (_, importPath) => {
